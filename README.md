...
![image](https://github.com/user-attachments/assets/9e17245c-efd1-483e-ac46-0b653da58ece)

Given this information, a logical player could conclude that there must be a mine in the lower-right cell and that there is no mine in the upper-left cell, for only in that case would the numerical labels on each of the other cells be accurate.

The goal of the game is to flag (i.e., identify) each of the mines. In this implementations of the game, the player can flag a mine by right-clicking on a cell.

[Minesweeper video](https://youtu.be/DAYEI0swOcw)

## Propositional Logic

I have built an AI that can play Minesweeper. Knowledge-based agents make decisions by considering their knowledge base, and making inferences based on that knowledge.
...
What information does the AI have access to? The AI would know every time a safe cell is clicked on and would get to see the number for that cell. Consider the following Minesweeper board, where the middle cell has been revealed, and the other cells have been labeled with an identifying letter for the sake of discussion.

![image](https://github.com/user-attachments/assets/e6f85fd4-3f68-4f6a-a3ec-f97538f76604)
...
![image](https://github.com/user-attachments/assets/6094f643-8faf-438d-9700-67722c588018)
...
![image](https://github.com/user-attachments/assets/44ddb329-9db6-4cc7-9866-04d45e27b007)
...
![image](https://github.com/user-attachments/assets/8e5cb7c8-ee8c-4569-b99d-e0ec3ac1cf7d)
...
In *minesweeper.py* there are three classes defined in this file, *Minesweeper*, which handles the gameplay; *Sentence*, which represents a logical sentence that contains both a set of cells and a count; and *MinesweeperAI*, which handles inferring which moves to make based on knowledge.

In the *Minesweeper* class each cell is a pair (i, j) where i is the row number (ranging from 0 to height - 1) and j is the column number (ranging from 0 to width - 1).

The *Sentence* class will be used to represent logical sentences of the form described above. Each sentence has a set of cells within it and a count of how many of those cells are mines. The class also contains functions *known_mines* and *known_safes* for determining if any of the cells in the sentence are known to be mines or known to be safe. It also contains functions *mark_mine* and *mark_safe* to update a sentence in response to new information about a cell.

The *MinesweeperAI* class will implement an AI that can play Minesweeper. The AI class keeps track of a number of values. *self.moves_made* contains a set of all cells already clicked on, so the AI knows not to pick those again. *self.mines* contains a set of all cells known to be mines. *self.safes* contains a set of all cells known to be safe. And *self.knowledge* contains a list of all of the *Sentences* that the AI knows to be true.
...
The *known_mines* function returns a set of all of the cells in *self.cells* that are known to be mines. The *known_safes* function  returns a set of all the cells in *self.cells* that are known to be safe. The *mark_mine* function first checks to see if cell is one of the cells included in the sentence. If cell is in the sentence, the function updates the sentence so that cell is no longer in the sentence, but still represents a logically correct sentence given that cell is known to be a mine. If cell is not in the sentence, then no action is necessary. The *mark_safe* function first checks to see if cell is one of the cells included in the sentence. If cell is in the sentence, the function updates the sentence so that cell is no longer in the sentence, but still represents a logically correct sentence given that cell is known to be safe. If cell is not in the sentence, then no action is necessary.

*add_knowledge* accepts a cell (represented as a tuple (i, j)) and its corresponding count, and updates *self.mines*, *self.safes*, *self.moves_made*, and *self.knowledge* with any new information that the AI can infer, given that cell is known to be a safe cell with count mines neighboring it. The function marks the cell as one of the moves made in the game. The function marks the cell as a safe cell, updating any sentences that contain the cell as well. The function adds a new sentence to the AI’s knowledge base, based on the value of cell and count, to indicate that count of the cell’s neighbors are mines. Be sure to only include cells whose state is still undetermined in the sentence. If, based on any of the sentences in *self.knowledge*, new cells can be marked as safe or as mines, then the function does so. If, based on any of the sentences in *self.knowledge*, new sentences can be inferred (using the subset method described in the Background), then those sentences are added to the knowledge base as well.

*make_safe_move* returns a move (i, j) that is known to be safe. The move returned is known to be safe, and not a move already made.
If no safe move can be guaranteed, the function returns None. The function not modifies *self.moves_made*, *self.mines*, *self.safes*, or *self.knowledge*. *make_random_move* returns a random move (i, j). This function will be called if a safe move is not possible: if the AI doesn’t know where to move, it will choose to move randomly instead. The move is not a move that has already been made.    The move is not be a move that is known to be a mine. If no such moves are possible, the function returns None.
